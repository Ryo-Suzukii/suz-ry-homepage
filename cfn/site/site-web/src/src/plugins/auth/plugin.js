...
import { createAuth0Proxy, createCustomProxy } from "./proxy/index.js"
  ...
  return {
    ...
    async checkLogin() {
      if (!isInitialized.value) {
        ...
        const loggedIn = await Promise.all([auth0.getUser(), custom.login(token)])
          .then(() => true)
          .catch(() => false)
        isInitialized.value = true
        return loggedIn
    ...
    async logout() {
      await Promise.all([custom.logout(), auth0.logout()])
    },
    async getToken() {
      return await auth0.getToken()
    },
    async getUser() {
