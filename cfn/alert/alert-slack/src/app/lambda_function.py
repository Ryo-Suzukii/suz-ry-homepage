...
from .clients import (
    AWSLambdaPowertoolsLog,
    AWSLambdaPowertoolsLogToSlackMessage,
    AWSLogToSlackMessage,
    AWSTextLogToSlackMessage,
)
...
def to_slack_messages(
    ...
    for event in data.log_events:
        ...
        if not message.keys() >= AWSLambdaPowertoolsLog.EXPECTED_KEYS:
            slack_messages.append(
                AWSTextLogToSlackMessage(
                    data.log_group,
                    data.log_stream,
...
def post_messages(slack_messages: list[AWSLogToSlackMessage]) -> list[SlackResponse]:
    def _post_message(
        slack_message: AWSLogToSlackMessage,
        channel: str = Env.SLACK_CHANNEL_ID,
    ) -> SlackResponse:
...
def lambda_handler(event: CloudWatchLogsEvent, _: LambdaContext) -> dict[str, int]:
